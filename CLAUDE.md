...
**Script-facing surface.** `LoadModule` registers **12 builtins** in this repo's source: `write`, `input`, `select`, `multi_select`, `confirm`, `note`, `md`, `md_note`, `spin`, `file_pick`, `colorize`, and `set_theme` (which **overrides** the auto-generated one so it re-applies the theme immediately). On top of those, `base.ConfigurableModule.LoadModule` auto-generates a `get_*`/`set_*` accessor pair for every non-secret config option: `get_width`/`set_width`, `get_height`/`set_height`, `get_theme`/`set_theme`, `get_editor`/`set_editor`. `doccov` only inspects this repo's `starlark.NewBuiltin` calls (the 12), but README must document the accessors too.
