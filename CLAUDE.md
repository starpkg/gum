...
**Verify on the go floor in Docker** — this repo's floor is **go 1.26.8** (see Release discipline), and may differ from the local toolchain. Behavior on the floor must be checked in a container:
...
- **`markdown.go`** — `starMarkdown` (`md`, `glamour` → ANSI), `starMarkdownNote` (`md_note`, renders then re-dispatches into `starNote`), and `starCodeBlock` (`code_block`, **non-interactive** `chroma` syntax highlight via `quick.Highlight` → ANSI).
...
- **`render.go`** — the **non-interactive** lipgloss v2 static renderers: `starStyle` (`style`), `starTable` (`table`), `starTree` (`tree`), `starCompose` (`compose`, lipgloss `JoinHorizontal`/`JoinVertical`). Unlike the huh-driven builtins these never open a TTY — they take data and return a rendered string, so they run headlessly. Colors reuse `ParseColor`; borders/alignment go through `parseBorder`/`parseAlign`/`parsePosition`; `style`'s padding/margin accept an int or list/tuple via `toIntList`.

**Script-facing surface.** `LoadModule` registers **18 builtins** in this repo's source: `write`, `input`, `select`, `multi_select`, `filter`, `confirm`, `note`, `md`, `md_note`, `code_block`, `spin`, `file_pick`, `colorize`, `style`, `table`, `tree`, `compose`, and `set_theme` (which **overrides** the auto-generated one so it re-applies the theme immediately). `style`/`table`/`tree`/`compose` are the non-interactive lipgloss renderers in `render.go`; `code_block` is the chroma syntax highlighter in `markdown.go`; `filter` is the fuzzy picker in `filter.go`. On top of those, `base.ConfigurableModule.LoadModule` auto-generates a `get_*`/`set_*` accessor pair for every non-secret, non-host-only config option: `get_width`/`set_width`, `get_height`/`set_height`, `get_theme`/`set_theme`. The `editor` option is **host-only** (`SetHostOnly(true)`): base generates `get_editor` but **no `set_editor`**, because the editor names the command `write` runs via huh's `os/exec` and a script must not choose it. `doccov` only inspects this repo's `starlark.NewBuiltin` calls (the 18), but README/`docs/API.md` must document the accessors too (keep `set_editor` as a "host-only, not generated" backtick token for the `doc-coverage-config` gate).
...
4. **Backward compatibility (iron rule).** `NewModule()` is the historical default surface (width 50, height 0, theme `charm`, empty editor). Any new config option or builtin must default to today's observable behavior — old scripts must run identically. `set_theme` must remain the gum-overriding version (immediate apply), not the plain `base` setter. *Exception:* the `editor` capability-gate deliberately breaks this — `write` lost its per-call `editor=` argument and `set_editor` is no longer generated, because a script NAMING the command huh runs via `os/exec` (Ctrl+E) is a sandbox escape. The editor command is host-configured only (`GUM_EDITOR` / `NewModuleWithConfig`), frozen at construction by `freezeEditor` (config → `$EDITOR` snapshot → `nano`, then rewritten to an absolute path via the host's PATH) and passed to huh's `.Editor()` — so a script can neither name it nor PATH-hijack it. This was an intentional, user-approved security change; don't "restore" script control of the editor. **Known residual (not closable in gum):** huh/bubbletea, not gum, construct the subprocesses they run, and gum cannot set their `Cmd.Env` (unlike the `cmd` module's `util.BuildChildEnv`). So a host exposing process-env mutation (e.g. a `runtime.setenv`) to untrusted scripts leaves two surfaces: (1) the **editor subprocess inherits the live env** → `VIMINIT` (and similar) execute for editors that honor them; (2) **bubbletea's color detection execs a bare `tmux`** (live PATH) on *any* form — including `spin`, whose huh spinner has no environment hook (so a per-form `tea.WithEnvironment` sanitize would be incomplete and was not added). **Do not add a process-wide `os.Setenv` "pin"** — it is racy against concurrent `runtime.setenv`, defeated by VIMINIT anyway, and gives false confidence (an earlier attempt was removed). The real fix is host/sandbox-level env isolation, tracked as an ecosystem-level follow-up (STAR item).
5. **Bounded static renders (DoS).** The `render.go` renderers turn script data into a string with no TTY, so a hostile script could otherwise drive lipgloss into a multi-gigabyte allocation. `tree` bounds nesting depth (`maxTreeDepth`); `style` bounds every dimension: `spacingInt` caps padding/margin at `maxStyleDimension` (clamping negatives to 0, at int64 precision), `iterValuesCapped` caps a spacing list at `maxSpacingValues`, and `styleAreaBound` caps the whole rendered area (output lines × output width) at `maxStyleCells`. `styleAreaBound` is a *conservative upper bound* on what lipgloss renders — it models wrap-vs-no-wrap at the effective width (`width − padding − border`), the natural-width overflow when a break unit can't fit (effWidth ≤ 1), word-wrap slack (2× the char-packing estimate), and tab/control-byte expansion (they advance one wrap unit but display up to `ctrlDisplayCells`). Its soundness is guarded by `TestStyleAreaBoundUpperBound`, which renders a large cross-product of adversarial inputs and asserts the bound never under-counts the actual render — **run it after any change to `styleAreaBound`/`styleWrapMetrics`/`lineWrapExtra`**. `table` and `compose` share the same amplification class and are bounded the same way: `tableAreaBound` (content-sized columns → Σ row heights × Σ column widths) and `composeAreaBound` (join pads to the widest/tallest block), each guarded by its own property test (`TestTableAreaBoundUpperBound`/`TestComposeAreaBoundUpperBound`). These property tests are the empirical oracle for the whole DoS-bound family — when a bound is hard to derive from first principles (third-party renderer output size), extend the property test's input cross-product rather than iterating reviews.
...
- **Floor = go 1.26.8**. The safe x/text dependency requires Go 1.26; require its current security patch. CI tests Go 1.26.x and 1.27.x.
