...
4. **Backward compatibility (iron rule).** `NewModule()` is the historical default surface (width 50, height 0, theme `charm`, empty editor). Any new config option or builtin must default to today's observable behavior — old scripts must run identically. `set_theme` must remain the gum-overriding version (immediate apply), not the plain `base` setter. *Exception:* the `editor` capability-gate deliberately breaks this — `write` lost its per-call `editor=` argument and `set_editor` is no longer generated, because a script NAMING the command huh runs via `os/exec` (Ctrl+E) is a sandbox escape. The editor command is host-configured only (`GUM_EDITOR` / `NewModuleWithConfig`), resolved by `resolveEditor` (config → `$EDITOR` snapshot at construction → `nano`) and passed to huh's `.Editor()`. This was an intentional, user-approved security change; don't "restore" script control of the editor. **Known residual (not closable in gum):** huh/bubbletea, not gum, construct the subprocesses they run (the editor on Ctrl+E, and a `tmux` probe during terminal detection on *any* form). They inherit the live process environment and resolve bare command names via the live `PATH`, so a host exposing process-env mutation (e.g. a `runtime.setenv`) to untrusted scripts can still steer them (PATH resolution, `VIMINIT`, the tmux probe). Unlike the `cmd` module (`util.BuildChildEnv`), gum cannot set those subprocesses' `Cmd.Env`/`Path`. **Do not add a process-wide `os.Setenv` "pin" to try to fix this** — it is racy against concurrent `runtime.setenv`, defeated by PATH/VIMINIT anyway, and gives false confidence (an earlier attempt was removed). The real fix is host/sandbox-level env isolation, tracked as an ecosystem-level follow-up.
