...
4. **Backward compatibility (iron rule).** `NewModule()` is the historical default surface (width 50, height 0, theme `charm`, empty editor). Any new config option or builtin must default to today's observable behavior — old scripts must run identically. `set_theme` must remain the gum-overriding version (immediate apply), not the plain `base` setter. *Exception:* the `editor` capability-gate deliberately breaks this — `write` lost its per-call `editor=` argument and `set_editor` is no longer generated, because a script choosing the command huh runs via `os/exec` is a sandbox escape (host command execution on Ctrl+E). The editor is host-configured only (`GUM_EDITOR` / `NewModuleWithConfig`), resolved by `resolveEditor` (config → `$EDITOR` snapshot at construction → `nano`). **huh's `NewText` reads the live `$EDITOR` itself — for the command *and* its arguments — and its `Editor()` setter can't clear leaked args**, so `newHostEditorText` pins `$EDITOR` to the resolved value (under `editorEnvMu`) for the duration of `NewText`; otherwise a script's `runtime.setenv` could inject editor args (e.g. `vim -c :!cmd` = code exec). This was an intentional, user-approved security change; don't "restore" script control of the editor, and don't drop the `$EDITOR` pin.
